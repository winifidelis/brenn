...
<div class="container position-relative">
    <div class="row">
        <img src="{{asset('assets_site/assets/images/banners/banner-busca.jpg')}}">
    </div>
</div>
...
<div class="container">
    ...
    <nav aria-label="breadcrumb" class="breadcrumb-nav">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#">Busca</a></li>
            <li class="breadcrumb-item"><a href="#">{{$produto_busca}}</a></li>
        </ol>
    </nav>
    ...
    <div class="row">
        <div class="col-lg-9 main-content">
            ...
            <form method="get" id="form_modifica_apresentacao" action="{{ route('modificarlistagemproduto') }}">
                ...
                <input type="hidden" name="produto_busca" id="produto_busca" value="{{$produto_busca}}">
                <input type="hidden" name="tipolista" id="tipolista" value="busca">

                <input type="hidden" name="getPorDepartamento" id="getPorDepartamento" value="{{$listagemprodutoordemdepartamento}}">
                <input type="hidden" name="getPorMarca" id="getPorMarca" value="{{$listagemprodutoordemmarca}}">
            </form>

            <script>
                function modificarQuantidadeApresentada(getDep, getMar) {
                    if (getDep != '') {
                        if (getDep.indexOf("XXX") != -1) {
                            //document.getElementById('removePorDepartamento').value = getDep;
                            getDep = getDep.replace('XXX ', '');
                            x = document.getElementById('getPorDepartamento').value;
                            x = x.replace("-*-" + getDep, '');
                            document.getElementById('getPorDepartamento').value = x;
                        } else {
                            document.getElementById('getPorDepartamento').value += "-*-" + getDep;
                        }
                    }
                    if (getMar != '') {
            ...
            <div class="row">
                ...
                <div class="col-6 col-sm-4">
                    <div class="product-default inner-quickview inner-icon">
                        <figure>
                            <a href="{{route('showproduto',$produto->id)}}">
                                <img src="{{asset('uploads/'.$produto->dirfotoprod)}}">
                            </a>
                            <div class="btn-icon-group">
                                <!--<button class="btn-icon btn-add-cart" data-toggle="modal" data-target="#addCartModal"><i class="icon-shopping-cart"></i></button>!-->
                                <a href="javascript:addItemAoCarrinhoQuick({{$produto->id}})" class="btn-icon btn-add-cart"><i class="icon-shopping-cart"></i></a>
                            </div>
                            <script>
                            ...
                            <a href="{{route('showprodutoquick',$produto->id)}}" class="btn-quickview" title="Quick View">Ver produto</a>
                        </figure>
                        <div class="product-details">
                            <div class="category-wrap">
                                <div class="category-list">
                                    <a href="{{route('showdepartamento', $produto->codigo_departamento)}}" class="product-category">{{$produto->descricaosite}}</a>
                                </div>
                                <!--
            ...
            {{ $produtosbusca->links('departamentoPaginacao') }}
        </div>
        ...
        <aside class="sidebar-shop col-lg-3 order-lg-first mobile-sidebar">
            <div class="sidebar-wrapper">
                @if ($listagemprodutoordemdepartamento != "" || $listagemprodutoordemmarca != "")
                <?php
                ...
                unset($listagemprodutoordemdepartamento[0]);
                ...
                <div class="widget">
                    ...
                    <div class="collapse show" id="widget-body-2">
                        <div class="widget-body">
                            <ul class="cat-list">
                                @foreach ($listagemprodutoordemdepartamento as $departamento)
                                <?php
                                $nomeSite = \App\Models\Produtodepartamento::select("descricaosite")
                                    ->where("descricao", "=", $departamento)
                                    ->first();
                                ?>
                                <li><a href="javascript:modificarQuantidadeApresentada('XXX {{$departamento}}','')">{{$nomeSite->descricaosite}} <b>[X]</b></a></li>
                                @endforeach
                ...
                <div class="widget">
                    <h3 class="widget-title">
                        <a data-toggle="collapse" href="#widget-body-2" role="button" aria-expanded="true" aria-controls="widget-body-2">Departamentos</a>
                    </h3>
                    <div class="collapse show" id="widget-body-2">
                        <div class="widget-body">
                            <ul class="cat-list">
                                @foreach ($departamentos as $departamento)
                                <li><a href="javascript:modificarQuantidadeApresentada('{{$departamento->departamento}}','')">{{$departamento->descricaosite}}</a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
